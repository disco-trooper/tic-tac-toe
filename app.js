...
const game = (() => {
  ...
  const isStartedVSAI = [false];
  const isStartedVSFriend = [false];
  const isStartedVSImpossible = [false];
  ...
  function listenForMoves() {
    document.addEventListener('click', (event) => {
      ...
      if (
        (event.target.getAttribute('id') == 'playVSFriend' ||
          event.target.getAttribute('id') == 'playVSAI' ||
          event.target.getAttribute('id') == 'playVSImpossible') &&
      ...
      if (
        event.target.getAttribute('id').substr(0, 4) == 'cell' &&
        isStartedVSFriend[0] == true
      ) {
        makeMoveVSFriend();
  ...
  function makeImpossibleMove() {
    const bestMove = minimaxModule.minimax(gameBoard.board, AI.choice).index;
  ...
  function makeAIMove() {
    ...
    while (
      gameBoard.board[randomNumber] == 'X' ||
      gameBoard.board[randomNumber] == 'O'
    ) {
      randomNumber = getRandomInt(9);
    }
    gameBoard.board[randomNumber] = AI.choice;
    document.querySelector(`${`#cell${randomNumber.toString()}`}`).textContent =
      AI.choice;
    checkOutcome();
    if (isStartedVSAI[0] == false) {
      buttons.setAttribute('style', 'display: block;');
  ...
  function makeMove() {
    const currentChoice = parseInt(
      event.target.getAttribute('id').substr(4, 1)
    );
  ...
  function checkRows() {
    for (let i = 0; i < 7; i += 3) {
      if (
        gameBoard.board[i] == playerOne.choice &&
        gameBoard.board[i + 1] == playerOne.choice &&
        gameBoard.board[i + 2] == playerOne.choice
      ) {
        announcePlayerOne();
      }
      if (
        gameBoard.board[i] == playerTwo.choice &&
        gameBoard.board[i + 1] == playerTwo.choice &&
        gameBoard.board[i + 2] == playerTwo.choice
      ) {
        announceP2AI();
      }
    }
  ...
  function checkColumns() {
    for (let i = 0; i < 3; i++) {
      if (
        gameBoard.board[i] == playerOne.choice &&
        gameBoard.board[i + 3] == playerOne.choice &&
        gameBoard.board[i + 6] == playerOne.choice
      ) {
        announcePlayerOne();
      }
      if (
        gameBoard.board[i] == playerTwo.choice &&
        gameBoard.board[i + 3] == playerTwo.choice &&
        gameBoard.board[i + 6] == playerTwo.choice
      ) {
        announceP2AI();
      }
    }
  ...
  function checkDiagonal() {
    if (
      (gameBoard.board[0] == playerOne.choice &&
        gameBoard.board[4] == playerOne.choice &&
        gameBoard.board[8] == playerOne.choice) ||
      (gameBoard.board[6] == playerOne.choice &&
        gameBoard.board[4] == playerOne.choice &&
        gameBoard.board[2] == playerOne.choice)
    ) {
      announcePlayerOne();
    }
    if (
      (gameBoard.board[0] == playerTwo.choice &&
        gameBoard.board[4] == playerTwo.choice &&
        gameBoard.board[8] == playerTwo.choice) ||
      (gameBoard.board[6] == playerTwo.choice &&
        gameBoard.board[4] == playerTwo.choice &&
        gameBoard.board[2] == playerTwo.choice)
    ) {
      announceP2AI();
    }
  }
...
const minimaxModule = (() => {
  ...
  const bestSpot = minimax(gameBoard.board, game.AI.choice);
  ...
  function minimax(newBoard, player) {
    ...
    if (winning(newBoard, game.AI.choice)) {
      return { score: 10 };
    }
    ...
    for (let i = 0; i < availSpots.length; i++) {
      ...
      if (player == game.AI.choice) {
        const result = minimax(newBoard, game.playerOne.choice);
        move.score = result.score;
      } else {
        const result = minimax(newBoard, game.AI.choice);
        move.score = result.score;
      }
